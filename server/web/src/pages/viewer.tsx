    ...
    (async () => {
      try {
        ...
        if (['jpg', 'jpeg', 'png', 'gif', 'webp', 'bmp'].includes(ext)) {
          const blob = new Blob([out], { type: `image/${ext === 'jpg' ? 'jpeg' : ext}` });
          setObjectUrl(URL.createObjectURL(blob));
        } else if (['mp4', 'webm'].includes(ext)) {
          const mime = ext === 'mp4' ? 'video/mp4' : 'video/webm';
          setObjectUrl(URL.createObjectURL(new Blob([out], { type: mime })));
        } else if (['txt', 'md', 'json', 'csv', 'log'].includes(ext)) {
          setTextPreview(new TextDecoder().decode(out));
        } else if (ext === 'pdf') {
          setObjectUrl(URL.createObjectURL(new Blob([out], { type: 'application/pdf' })));
        }
      } catch (e) {
